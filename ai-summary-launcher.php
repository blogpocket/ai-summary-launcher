 ...
 * Description:       Muestra iconos de Claude, ChatGPT, Google AI (Gemini), Grok y Perplexity para abrir su interfaz con un prompt para resumir el artículo actual. Incluye copia del prompt al portapapeles antes de abrir la IA.
 * Version:           1.0.2
...
function aisl_default_options() {
    // Build default post types: all public post types except 'page'
...
function aisl_get_options() {
    ...
    if ( empty( $merged['apply_post_types'] ) ) {
        // If user cleared all, respect that (no auto insert anywhere), keep array empty
        $merged['apply_post_types'] = array();
    }
    return $merged;
}
...
function aisl_render_buttons( $post_id = null ) {
    ...
    $services = array(
        'chatgpt'    => array( 'label' => 'ChatGPT',     'url' => 'https://chatgpt.com/?q=',          'svg' => 'chatgpt.svg',     'prefill_supported' => true ),
...
/** Auto-insert to content only for selected post types (exclude pages by default) */
function aisl_maybe_append_to_content( $content ) {
    ...
    if ( ! in_array( $pt, (array) $opts['apply_post_types'], true ) ) {
        return $content; // do not auto-insert on non-selected PTs (e.g. pages)
    }
...
function aisl_sanitize_options( $input ) {
    ...
    // Position
    $out['position'] = ( isset( $input['position'] ) && in_array( $input['position'], array( 'before', 'after', 'manual' ), true ) )
        ? $input['position'] : $defaults['position'];

    // Target
    $out['open_target'] = ( isset( $input['open_target'] ) && in_array( $input['open_target'], array( '_blank', '_self' ), true ) )
        ? $input['open_target'] : $defaults['open_target'];

    // Behavior
    $out['copy_behavior'] = ( isset( $input['copy_behavior'] ) && in_array( $input['copy_behavior'], array( 'prefill_and_copy', 'copy_only', 'prefill_only' ), true ) )
        ? $input['copy_behavior'] : $defaults['copy_behavior'];
    ...
    // Note: if user no selecciona ninguno, se guarda lista vacía (no auto-insert en ningún PT)
